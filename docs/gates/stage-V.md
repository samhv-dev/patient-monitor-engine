...
Branch `stage-v-ventilator-link`, based on `main` `69731e8` (Stage 4b merged at `d96c8c0`; `main` has since gained only `f45ba96`, docs), built from `docs/plans/stage-v-ventilator-link.md` (21 tasks). Rulings applied: R27, R35, R36, R39 §5, R41 (all plan choices accepted incl. C1; file rule for the data table).

| Check | Result |
|---|---|
| typecheck / `pnpm -r test` / build / check-notices / e2e | see Task 21 run below; ventilator **87** (13 files); engine-core **447** (444 on this base + 3 new; the plan's 372 predates Stage 4b); `check-notices: OK`; `vent-link.e2e.ts` **2 passed** (1.5 min) |
...
7. **Task 17/18 on the 4b base** — no find/replace misses: `apps/demo/package.json`, `vite.config.ts` and `index.html` anchors matched once; renderer's `mountMonitor` options used by the page are unchanged by 4b; every created file is identical to the plan author's tree except the fixes above (and a trailing blank line in `in-process.ts`).
