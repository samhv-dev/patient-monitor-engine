...
*Branch `stage-4b-device-layer`. Plan: `docs/plans/stage-4b-device-layer.md`, all 26 tasks ticked. Base: `origin/main` 601b410, which includes Stage 2 (final), Stage 6b and the RESUME runbook. Stage 3 was then **merged in** (main 32b539f): see "Merge of Stage 3". The plan was written against 121c3f4 + Stage 2 52efd04. Every edit block of the plan still applied exactly (0 misses). The drift that did matter was Stage 6b: see "Deviations".*

## What landed

- **Alarm engine** (`packages/engine-core/src/l3/alarms/**`, `l3/device-layer.ts`):
  - Conditions: limits on displayed numerics by age band, desaturation, asystole, VF, VT runs, brady/tachy or extreme brady/tachy, pause, PVCs/min, leads off, SpO2 probe off, NIBP failed, and (added at execution) APNEA and the CO2 line INOP. APNEA comes from Stage 3's capnograph and impedance detectors, re-issued with their own ids (`apnoea-co2`, `apnoea-resp`).
...
- Audio, live page on zoll-like with VF, charge 120 J and shock: the scheduler log has **10 alarm pulses, 1 charge, 1 chargeReady and 1 shock** tone. The largest alarm-pulse lateness is **28.0 ms** (34; 33.0 ms before the Stage 3 merge), under the test's 150 ms bound. See `docs/gates/stage-4b/audio-timing.json`.

### Merge of Stage 3 and alarm wiring on real Stage 3 events

`origin/main` 32b539f (Stage 3, PR #8) was merged into the branch at abd8cae.

**Conflicts and how they were resolved.** Both sides were kept throughout:
- `engine-core/src/index.ts`, `types.ts`: both export lists are kept. The `Command` and `EngineEvent` unions now carry both `DeviceClinicalEvent`/`DeviceEvent` and `RespCommandBody`/`RespEvent`.
- `engine.ts`: both imports are kept. Commands route to the device layer first, then Stage 3's resp pipeline, then Stage 2's hemo pipeline, in both validate and apply. The three accept disjoint commands.
- `renderer/src/index.ts`: both export lists are kept.
- `monitor-core.ts`: the 4b plan-driven core is kept (it already reads per-lane sample rate and sweep speed from the plan). Stage 3's changes are ported into it: resp is auto-ranged over 10 s with `RESP_MIN_SPAN` 0.5, pleth over 4 s, and the scratch buffer holds 1250 samples.
- `mount.ts`: the 4b skin path is kept, and Stage 3's SpO2/EtCO2/RR/TEMP tiles are added to the legacy (no-skin) path. With a skin, DeviceUI's own tiles show those numerics.
- `skin-plan.ts` (a compile break, not a textual conflict): `WAVE_LANE` gains `co2`/`resp`, and the legacy plan takes Stage 3's sweep speeds from `WAVE_STYLE` (6.25 mm/s).
- Auto-merged without conflict: `apps/demo/vite.config.ts` and `index.html` keep every page. `NOTICES.md` was not touched by 4b.

**Wiring to the real Stage 3 events.** Measured on seed 7, adult, ventilated at RR 12, mainstream capnograph:
- **Apnoea.** Stage 3 emits level-less raw alarms (`apnoea-co2` from the capnograph's awRR detector, `apnoea-resp` from impedance). The device layer drops every level-less alarm, so the alarm manager re-issues them with the SAME ids, level 1, high, and the skin's text: `***APNEA` / `***APNEA (RESP)` (IEC-style), `CO2 APNEA` / `RESP APNEA` (Saadat-like). This follows the ecgLeadsOff / nibp-failed pattern of plan decision 8.
  - On ventilator disconnection, the alarm was raised **20.00 s after the last CO2 upstroke** on both philips-like and saadat-like, and it appears in `alarmStatus`.
  - Stage 3's own acceptance tests, which look for `apnoea-co2`, pass unchanged.
  - The breath-gap APNEA of the synthetic-hooks commit was dropped. It duplicated the detectors, and the monitor should alarm on what its sensors see.
- **SpO2 limit.** With target 86, philips-like raised `**SpO2 85<90`, L2, at 65.02 s. The first displayed SpO2 under 90 was at 55 s, so the skin's 10 s SpO2 delay applied.
- **EtCO2 limit.** With target 62, `**etCO2 63>50`, L2, was raised on the first displayed value over 50 (36 s).
- **Desaturation.** With target 70, `***DESAT` was raised **20.00 s** after the first displayed value under 80 (brief §6.4).
- **Post-shock EtCO2 rise.** EtCO2 was 41 mmHg before VF and 0 at the end of 110 s of VF, ventilated without CPR. After a shock converted to sinus by the instructor's pre-selection it peaked at **49 mmHg**: Stage 3's CO2 washout on the restored cardiac output. The device layer needs no code for it, because Stage 3 couples CO2 elimination to cardiac output.
- **CO2 line INOP.** Stage 3 emits no line/occlusion flag. The `co2Line` hook is ready and unit-tested, and R-4b-1 stays open for that flag only.
- **Tests on the real pipeline**: `test/engine/stage3-alarms-engine.test.ts` (3 tests). `stage3-hooks.test.ts` was rewritten for the same-id re-issue (5 tests). In `hemo-engine.test.ts`, the `attachSensor ecg` expectation now follows 4b (accepted).
- **Renderer**: on the live page, RESP draws in the saadat-like RESP lane, and SpO2 (96) and TEMP (36.8) fill the skin tiles. CO2 stays dashed on the demo page, which attaches no capnograph.

### Screenshots (`docs/gates/stage-4b/`, 14 PNGs, all ≤ 34 KB)

- `saadat-like--idle.png`: grey idle bar and red crossed bells in the header and in the HR, NIBP, IBP1, IBP2, SpO2, TEMP and RR tiles. The green `II  X1  NORMAL` lane, magenta PLETH, salmon IBP1 (200/40 scale), light-blue IBP2 (30/−10), and since the Stage 3 merge a yellow RESP trace. The TEMP tile reads 36.8 and IBP1 `117/76`; a three-part pressure now drops to 32 px so it fits the column.
...
- APNEA uses Stage 3's detectors, which run at a fixed 20 s. The skin's apnoea time (saadat-like RESP 10 s) is not applied yet (R-4b-9). The CO2 line INOP waits for a Stage 3 flag (R-4b-1).
...
| ID | Owner | Request |
|---|---|---|
| R-4b-1 | Stage 3 (follow-up) | Only the CO2 line/occlusion INOP flag is still open: emit a raw `co2Line` alarm, which 4b re-issues as `CO2 LINE` / `CO2 CHECK LINE`. Breath events, apnoea flags, the EtCO2 rise and the numerics are wired |
...
| R-4b-9 | Stage 3 / 5.1 | Let the apnoea detectors take the skin's apnoea time (`DeviceProfile.apneaS`: saadat-like RESP 10 s, gas 20 s; IEC-style 20 s) instead of a fixed 20 s |
...
- **Extra commit bf3678c (executor brief, not in the plan): Stage 3 alarm hooks.**
  ...
  - Inputs: `lastBreathT` from brief §7.3 `breath` events. This input was removed again at the Stage 3 merge, in favour of the detectors.
  ...
  - Conditions: APNEA at level 1, evaluated whatever the switches say, with the texts `***APNEA` and `RESP APNEA` (at the Stage 3 merge this became the same-id re-issue of `apnoea-co2` / `apnoea-resp`); and `co2Line` at level 3, technical, with `CO2 LINE` and `CO2 CHECK LINE` [inferred].
...
- **Stage 3 merge (renderer)**: a three-part pressure such as `117/76` overflowed the 180 px Saadat-like IBP1 tile at 40 px, showing `117/2`. DeviceUI now drops values longer than 5 characters to 32 px (48 px in a large tile) [ENG].
