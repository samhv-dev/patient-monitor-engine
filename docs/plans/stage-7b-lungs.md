...
> **Executor note (2026-09-26, after 7a merged as 1fd0851):** Task 26 then RAN. 7a had already applied pvrLung/pvrLungL/pvrLungR in control() (its R46 seam), so model.ts needed no edit. respPleural deviates from the plan body: it keeps 7a's continuous pleural shape (pleuralPressureMmHg, calibrated T_IT 0.65) and scales its swing by the condition's tIt relative to the healthy 0.4 (the plan's absolute `tIt·Palv` would have dropped a healthy patient from 7a's 0.65 to 0.4 and moved every 7a/Stage 2 PPV number), adds k·T_IT·auto-PEEP on the internal ventilator (external frames already carry the ventilator's palv), and max-combines the lungs' pPtx with 7a's own ext.pPtx (new `circPtx` in circ-link.ts) so a scenario sending both commands does not count it twice. lung-circ test: OLV isolated-lung PVR ×1.91, flow share 0.300 at 30 min; the COPD test runs in MODELED mode (MANUAL trackers hold MAP) and asserts CO −23 % (4.35 → 3.35) and a MAP fall (101.8 → 98.9, −2.8 %; the plan's ≥ 10 % is not reached with the baroreflex — needs a ruling). Stage V link tests (7a NR-3): massive PE flipped to a real test (the scenario also sends the lung's `pe` condition: EtCO2 38 → 24.6); COPD GOLD 3–4 flipped with auto-PEEP in R46's 6–12 and MAP fall by direction (> 3); cardiogenic oedema stays it.fails (an `hfref` profile condition was tried: CO 5.08 vs < 4.99 needed); ARDS PEEP 5 → 15 → 5 SpO2 fall at the band edge on 7a + 7b (−3.0 vs > 3; 7a alone −3.4) → it.fails + needs a ruling. The 24 h lung test uses LONGRUN_HOURS.
